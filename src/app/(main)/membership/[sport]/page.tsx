...
export default async function page({
  ...
  return (
    <main className="py-10 container">
      <Image
        src={`/imgs/sports/${sport?.image}`}
        alt={sport?.description as string}
        width={2000}
        height={500}
        className="absolute h-52 w-11/12 object-cover -z-10 left-1/2 -translate-x-1/2"
      />
              ...
              {sport?.info
                ...
                .map((info, idx) =>
                  info.plan.map((plan, pIdx) => (
                    <div
                      key={`${idx}-${pIdx}`}
                      className="border border-gray-600 p-4 bg-white/5 flex flex-col gap-4"
                    >
                      <div>
                      ...
                      <h4>
                        {plan.amount} {plan.amount > 10 ? "تمرينة" : "تمرينات"}{" "}
                        في الشهر
                      </h4>
                    </div>
                  ))
